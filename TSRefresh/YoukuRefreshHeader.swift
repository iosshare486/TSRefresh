...
class YoukuRefreshHeader: TSRefreshHeader, TSRefreshHeaderProtocol {
...
//    }
    ...
    func tsToWillRefreshState() {
        if !self.backgroundImageView.isHidden {
            self.iconImageView.isHidden = true
            self.rotatingImageView.isHidden = true
        }
    }
    func tsChangePullingPercent(percent: CGFloat) {
        print("\(percent)")
    }
    func tsWillBeginEndRefershing(isSuccess: Bool) {
        self.rotatingImageView.isHidden = true
    ...
    func tsWillCompleteEndRefershing() {
